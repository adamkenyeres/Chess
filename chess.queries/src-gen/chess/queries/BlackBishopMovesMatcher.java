...
/**
 * Generated pattern matcher API of the chess.queries.blackBishopMoves pattern,
 * providing pattern-specific query methods.
 * 
 * <p>Use the pattern matcher on a given model via {@link #on(ViatraQueryEngine)},
 * e.g. in conjunction with {@link ViatraQueryEngine#on(Notifier)}.
 * 
 * <p>Matches of the pattern will be represented as {@link BlackBishopMovesMatch}.
 * 
 * <p>Original source:
 * <code><pre>
 ...
 * @see BlackBishopMovesQuerySpecification
 * 
 */
@SuppressWarnings("all")
public class BlackBishopMovesMatcher extends BaseMatcher<BlackBishopMovesMatch> {
  /**
   ...
   * 
   */
  public static BlackBishopMovesMatcher on(final ViatraQueryEngine engine) throws ViatraQueryException {
    // check if matcher already exists
    BlackBishopMovesMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = new BlackBishopMovesMatcher(engine);
  ...
  private final static Logger LOGGER = ViatraQueryLoggingUtil.getLogger(BlackBishopMovesMatcher.class);
   ...
   * 
   */
  private BlackBishopMovesMatcher(final ViatraQueryEngine engine) throws ViatraQueryException {
   ...
   * @return matches represented as a BlackBishopMovesMatch object.
   * 
   */
  public Collection<BlackBishopMovesMatch> getAllMatches(final Bishop pPiece, final Square pSquare, final Chess pChess) {
   ...
   * @param pSquare the fixed value of pattern parameter square, or null if not bound.
   * @param pChess the fixed value of pattern parameter chess, or null if not bound.
   * @return a match represented as a BlackBishopMovesMatch object, or null if no match is found.
   * 
   */
  public BlackBishopMovesMatch getOneArbitraryMatch(final Bishop pPiece, final Square pSquare, final Chess pChess) {
   ...
   * 
   */
  public void forEachMatch(final Bishop pPiece, final Square pSquare, final Chess pChess, final IMatchProcessor<? super BlackBishopMovesMatch> processor) {
   ...
   * 
   */
  public boolean forOneArbitraryMatch(final Bishop pPiece, final Square pSquare, final Chess pChess, final IMatchProcessor<? super BlackBishopMovesMatch> processor) {
   ...
   * 
   */
  public BlackBishopMovesMatch newMatch(final Bishop pPiece, final Square pSquare, final Chess pChess) {
    return BlackBishopMovesMatch.newMatch(pPiece, pSquare, pChess);
  }
   ...
   * 
   */
  public Set<Bishop> getAllValuesOfpiece(final BlackBishopMovesMatch partialMatch) {
   ...
   * 
   */
  public Set<Square> getAllValuesOfsquare(final BlackBishopMovesMatch partialMatch) {
   ...
   * 
   */
  public Set<Chess> getAllValuesOfchess(final BlackBishopMovesMatch partialMatch) {
  ...
  @Override
  protected BlackBishopMovesMatch tupleToMatch(final Tuple t) {
    try {
    	return BlackBishopMovesMatch.newMatch((Bishop) t.get(POSITION_PIECE), (Square) t.get(POSITION_SQUARE), (Chess) t.get(POSITION_CHESS));
  ...
  @Override
  protected BlackBishopMovesMatch arrayToMatch(final Object[] match) {
    try {
    	return BlackBishopMovesMatch.newMatch((Bishop) match[POSITION_PIECE], (Square) match[POSITION_SQUARE], (Chess) match[POSITION_CHESS]);
    } catch(ClassCastException e) {
  ...
  @Override
  protected BlackBishopMovesMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return BlackBishopMovesMatch.newMutableMatch((Bishop) match[POSITION_PIECE], (Square) match[POSITION_SQUARE], (Chess) match[POSITION_CHESS]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
   ...
   * 
   */
  public static IQuerySpecification<BlackBishopMovesMatcher> querySpecification() throws ViatraQueryException {

...
/**
 * A pattern-specific query specification that can instantiate TransitiveClosureWestWithPiecesMatcher in a type-safe way.
 * 
 * @see TransitiveClosureWestWithPiecesMatcher
 * @see TransitiveClosureWestWithPiecesMatch
 * 
 */
@SuppressWarnings("all")
public final class TransitiveClosureWestWithPiecesQuerySpecification extends BaseGeneratedEMFQuerySpecification<TransitiveClosureWestWithPiecesMatcher> {
  private TransitiveClosureWestWithPiecesQuerySpecification() {
   ...
   * 
   */
  public static TransitiveClosureWestWithPiecesQuerySpecification instance() throws ViatraQueryException {
    try{
  ...
  @Override
  protected TransitiveClosureWestWithPiecesMatcher instantiate(final ViatraQueryEngine engine) throws ViatraQueryException {
    return TransitiveClosureWestWithPiecesMatcher.on(engine);
  }
  
  @Override
  public TransitiveClosureWestWithPiecesMatch newEmptyMatch() {
    return TransitiveClosureWestWithPiecesMatch.newEmptyMatch();
  }
  
  @Override
  public TransitiveClosureWestWithPiecesMatch newMatch(final Object... parameters) {
    return TransitiveClosureWestWithPiecesMatch.newMatch((chessdiagram.Square) parameters[0], (chessdiagram.Square) parameters[1], (chessdiagram.Square) parameters[2]);
  }
  
  /**
   * Inner class allowing the singleton instance of {@link TransitiveClosureWestWithPiecesQuerySpecification} to be created 
   * 	<b>not</b> at the class load time of the outer class, 
   * 	but rather at the first call to {@link TransitiveClosureWestWithPiecesQuerySpecification#instance()}.
   * 
   * <p> This workaround is required e.g. to support recursion.
   * 
   */
  private static class LazyHolder {
    private final static TransitiveClosureWestWithPiecesQuerySpecification INSTANCE = new TransitiveClosureWestWithPiecesQuerySpecification();
  ...
  private static class GeneratedPQuery extends BaseGeneratedEMFPQuery {
    private final static TransitiveClosureWestWithPiecesQuerySpecification.GeneratedPQuery INSTANCE = new GeneratedPQuery();
    
    @Override
    public String getFullyQualifiedName() {
      return "chess.queries.transitiveClosureWestWithPieces";
    }
      ...
      try {
      	{
      		...
      		// 	find W +(start, square)
      		new BinaryTransitiveClosure(body, new FlatTuple(var_start, var_square), WQuerySpecification.instance().getInternalQueryRepresentation());
      		...
      		new ExpressionEvaluation(body, new IExpressionEvaluator() {
      		                            ...
      		                            public String getShortDescription() {
      		                                return "Expression evaluation from pattern transitiveClosureWestWithPieces";
      		                            }
